...
var Loading = {
    ...
    loadLogoImage: function () {
        ...
        this.logoImage.src = "splash-screens/johnbutlergames.png";
        this.logoImage.onload = function () {
            Loading.loads++;
        }
        this.logoImage2 = document.createElement("img");
        this.logoImage2.src = "splash-screens/coolmathgames.png";
        this.logoImage2.onload = function () {
            Loading.loads++;
        }
    },
    updateLoadingScreen: function () {
        var s = Math.min(dimensions.width, dimensions.height) / 500;
        var percent = this.loads / (Object.keys(images).length + Object.keys(audios).length + 2);
        ...
        if (percent == 1) {
            var a = 1 - this.percent * (Object.keys(images).length + Object.keys(audios).length + 2) + this.loads - 1;
            ctx.globalAlpha = a;
        }
        ctx.fillStyle = "black";
    ...
    updateIntro: function () {
        ...
        var i3 = Math.min(Physics.easeInOut((this.intro - 300) / 25), Physics.easeInOut((200 - (this.intro - 300)) / 25));
        var i4 = Math.min(Physics.easeInOut((this.intro - 300) / 25), Physics.easeInOut((200 - (this.intro - 300)) / 25));
        ctx.save();
        ctx.translate(dimensions.width / 2, dimensions.height / 2);
        ctx.scale(s * i2, s * i2);
        ctx.translate(-250, -250);
        ...
        ctx.save();
        ctx.translate(dimensions.width / 2, dimensions.height / 2);
        ctx.scale(s * (i4 * 0.2 + 0.8), s * (i4 * 0.2 + 0.8));
        ctx.translate(-250, -250);
        ctx.globalAlpha = i3;
        ctx.drawImage(this.logoImage2, 0, 0, 500, 500);
        ctx.restore();

        this.intro++;
        if (this.intro == 550) this.intro = 0;
    }
}
var Mouse = {
    x: 0,
